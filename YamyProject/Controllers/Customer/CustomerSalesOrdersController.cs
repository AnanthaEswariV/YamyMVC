namespace YamyProject.Controllers.Customer
    {
    public class CustomerSalesOrdersController(ISalesOrderCenterService SalesOrder, IListServices listServices, YamyDbContext context) : Controller
        {
        private readonly ISalesOrderCenterService _SalesOrder = SalesOrder;
        private readonly IListServices _ListServices = listServices;
        private readonly YamyDbContext _Context = context;

        public async Task<IActionResult> Index()
            {
            var Sales = await _SalesOrder.GetSalesAsync();
        ...
        public async Task<IActionResult> Create()
            ...
            return View("SalesOrder",
                new TaxInvoiceViewModel
                    {
                    Invoce = _SalesOrder.GenerateInvoiceNoAsync().GetAwaiter().GetResult(),
                    NextCode = _SalesOrder.GenerateInvoiceNoAsync().GetAwaiter().GetResult(),
        ...
        public async Task<IActionResult> Edit(int id)
            {
            var vm = await _SalesOrder.GetEditAsync(id);
            return View("SalesOrder", vm);
            }
        [HttpPost]
        ...
        public async Task<IActionResult> CreateQuotationFromProforma(int id)
            {
            var vm = await _SalesOrder.GetSalesProformaDataAsync(id);
            return View("SalesOrder", vm);
            }
        //[HttpPost]
        ...
        public async Task<IActionResult> GetItemsDetels(int Id)
            {
            var result = await _Context.TblSalesOrderDetails
