namespace YamyProject.Controllers.Vendors
    {
    public class PurchaseOrdersController(IPurchaseOrdersService PurchaseOrderService, IListServices listServices, YamyDbContext context) : Controller
        {
        private readonly IPurchaseOrdersService _PurchaseOrderService = PurchaseOrderService;
        private readonly IListServices _ListServices = listServices;
        private readonly YamyDbContext _Context = context;
        public async Task<IActionResult> Index()
            {
            var Purchase = await _PurchaseOrderService.GetPurchaseAsync();
            var customers = await _ListServices.GetVendorsAsync();
            var customerSelectList = customers
                ...
                .ToList();
            var Purchas = new PurchasesCenterViewModel
                {
                Purchases = Purchase,
                Vendors = customerSelectList
                };
            return View(Purchas);
            }
        ...
        public async Task<IActionResult> Create()
            ...
            return View("PurchaseOrder",
                new PurchaseInvoiceViewModel
                    {
                    Invoce = _PurchaseOrderService.GenerateVendorsInvoiceNoAsync().GetAwaiter().GetResult(),
                    NextCode = _PurchaseOrderService.GenerateVendorsInvoiceNoAsync().GetAwaiter().GetResult(),
        ...
        public async Task<IActionResult> Edit(int id, string formType = "")
            {
            var vm = await _PurchaseOrderService.GetEditAsync(id, formType);
