    ...
    ORDER BY code";
        ...
        public async Task<IActionResult> SaveOrUpdateEstimation([FromBody] GenerateEstimateRequest model)
        {
            ...
            try
            {
                ...
                // ✅ Add AllowBatch=true to support multiple statements
                ...
                try
                {
                    // ── Check if estimate already generated ─────────────────────────
                    if (model.Id > 0)
                    {
                        string checkEstimate =
                            "SELECT estimate_status FROM tbl_project_tender WHERE id=@id";
                        await using var checkCmd = new MySqlCommand(checkEstimate, conn,
                ...
                WHERE id = @id;";
                    ...
                    // ── ✅ FIX: Split deletes into individual commands (safe order) ──
                    // Order matters: delete child rows BEFORE parent rows

                    // 1. Delete assembly_bos first (references tbl_items_boq ids)
                    string deleteBosBosQuery = @"
                DELETE tab FROM tbl_item_assembly_bos tab
                INNER JOIN tbl_items_boq boq ON boq.id = tab.assembly_id
                WHERE boq.ref_id = @tenderId;";
                    await using var del1 = new MySqlCommand(deleteBosBosQuery, conn, transaction);
                    del1.Parameters.AddWithValue("@tenderId", model.Id);
                    await del1.ExecuteNonQueryAsync();

                    // 2. Delete boq_details (references tbl_items_boq ids)
                    string deleteBoqDetailsQuery = @"
                DELETE tbd FROM tbl_items_boq_details tbd
                INNER JOIN tbl_items_boq boq ON boq.id = tbd.ref_id
                WHERE boq.ref_id = @tenderId;";
                    await using var del2 = new MySqlCommand(deleteBoqDetailsQuery, conn, transaction);
                    del2.Parameters.AddWithValue("@tenderId", model.Id);
                    await del2.ExecuteNonQueryAsync();

                    // 3. Delete tender details
                    await using var del3 = new MySqlCommand(
                        "DELETE FROM tbl_project_tender_details WHERE tender_id = @tenderId;",
                        conn, transaction);
                    del3.Parameters.AddWithValue("@tenderId", model.Id);
                    await del3.ExecuteNonQueryAsync();

                    // 4. Delete parent BOQ rows
                    await using var del4 = new MySqlCommand(
                        "DELETE FROM tbl_items_boq WHERE ref_id = @tenderId;",
                        conn, transaction);
                    del4.Parameters.AddWithValue("@tenderId", model.Id);
                    await del4.ExecuteNonQueryAsync();

                    // 5. Delete transactions
                    await using var del5 = new MySqlCommand(
                        "DELETE FROM tbl_transaction WHERE type='Project Tender' AND transaction_id=@tenderId;",
                        conn, transaction);
                    del5.Parameters.AddWithValue("@tenderId", model.Id);
                    await del5.ExecuteNonQueryAsync();

                    // 6. Delete item card details
                    await using var del6 = new MySqlCommand(
                        "DELETE FROM tbl_item_card_details WHERE trans_type='Project Tender' AND trans_no=@tenderId;",
                        conn, transaction);
                    del6.Parameters.AddWithValue("@tenderId", model.Id);
                    await del6.ExecuteNonQueryAsync();

                    // ── Get next item code ───────────────────────────────────────────
                    await using var codeCmd = new MySqlCommand(
                        "SELECT IFNULL(MAX(CAST(code AS UNSIGNED)), 0) + 1 FROM tbl_items;",
                        conn, transaction);
                    int itemCode = Convert.ToInt32(await codeCmd.ExecuteScalarAsync() ?? 1);
                    ...
                    foreach (var item in model.Items)
                    {
                        ...
                        string marginPct = string.IsNullOrEmpty(item.MarginPercentage) ? "0" : item.MarginPercentage;
                    ...
                    INSERT INTO tbl_items_boq
                        (sr, ref_id, type, name, unit_name, qty, price, amount,
                         length, width, thickness, note)
                    VALUES
                        (@sr, @tenderId, @type, @itemName, @unit, @qty, @rate, @amount,
                         @length, @width, @thickness, @note); 
                    SELECT LAST_INSERT_ID();";
                        ...
                        if (boqItemId <= 0)
                            throw new Exception($"Failed to insert BOQ row for item: {description}");

                        // ── Insert tender details ────────────────────────────────────
                        string insertDetailsQuery = @"
                    INSERT INTO tbl_project_tender_details 
                        (sr, tender_id, item_id, qty, unit_id, rate, amount,
                         length, width, thickness, note, margin_percentage, margin_amount)
                    VALUES 
                        (@sr, @tenderId, @itemId, @qty, @unitId, @rate, @amount,
                         @length, @width, @thickness, @note, @marginPct, @marginAmt);";
                        ...
                        detailsCmd.Parameters.AddWithValue("@marginPct", marginPct);
                        detailsCmd.Parameters.AddWithValue("@marginAmt", marginAmount);
                        await detailsCmd.ExecuteNonQueryAsync();

                        // ── Process assembly items for this BOQ row ──────────────────
                        if (item.AssemblyItems != null && item.AssemblyItems.Count > 0)
                        {
                            Console.WriteLine($"[Assembly] Item '{description}' has {item.AssemblyItems.Count} assembly items");

                            int subId = 1;
                            foreach (var assemblyItem in item.AssemblyItems)
                            {
                                string asmCode = sr + subId;
                                string asmName = !string.IsNullOrEmpty(assemblyItem.Name)
                                                 ? assemblyItem.Name
                                                 : description;

                                decimal asmCost = decimal.TryParse(
                                    assemblyItem.Cost.ToString(),
                                    NumberStyles.Any, CultureInfo.InvariantCulture, out var c) ? c : 0;
                                decimal asmQty = decimal.TryParse(
                                    assemblyItem.Qty.ToString(),
                                    NumberStyles.Any, CultureInfo.InvariantCulture, out var q) ? q : 0;
                                decimal asmTotal = decimal.TryParse(
                                    assemblyItem.Total.ToString(),
                                    NumberStyles.Any, CultureInfo.InvariantCulture, out var t) ? t : 0;

                                Console.WriteLine($"  → [{subId}] Name:{asmName} Qty:{asmQty} Cost:{asmCost}");

                                // ── Step 1: Insert tbl_items_boq_details ────────────
                                string insertBoqDetailsQuery = @"
                            INSERT INTO tbl_items_boq_details(
                                code, warehouse_id, type, category_id, name, unit_id,
                                barcode, cost_price, cogs_account_id, vendor_id,
                                sales_price, income_account_id, asset_account_id,
                                min_amount, max_amount, on_hand, method, total_value,
                                date, img, active, state, created_By, created_date, ref_id)
                            VALUES(
                                @code, @warehouseId, @type, 0, @name, 0,
                                '', @costPrice, @cogsAccountId, @vendorId,
                                0, @incomeAccountId, @assetAccountId,
                                0, 0, @onHand, 'fifo', 0,
                                @date, '', 0, 0, @createdBy, @createdDate, @refId);
                            SELECT LAST_INSERT_ID();";

                                await using var boqDetailsCmd = new MySqlCommand(
                                    insertBoqDetailsQuery, conn, transaction);
                                boqDetailsCmd.Parameters.AddWithValue("@code", asmCode);
                                boqDetailsCmd.Parameters.AddWithValue("@warehouseId", model.WarehouseId);
                                boqDetailsCmd.Parameters.AddWithValue("@type", "13 - Inventory Assembly");
                                boqDetailsCmd.Parameters.AddWithValue("@name", asmName);
                                boqDetailsCmd.Parameters.AddWithValue("@costPrice", asmCost);
                                boqDetailsCmd.Parameters.AddWithValue("@cogsAccountId", assemblyItem.COGSAccountId);
                                boqDetailsCmd.Parameters.AddWithValue("@vendorId", assemblyItem.VendorAccountId);
                                boqDetailsCmd.Parameters.AddWithValue("@incomeAccountId", assemblyItem.IncomeAccountId);
                                boqDetailsCmd.Parameters.AddWithValue("@assetAccountId", assemblyItem.AssetAccountId);
                                boqDetailsCmd.Parameters.AddWithValue("@onHand", asmQty);
                                boqDetailsCmd.Parameters.AddWithValue("@date", model.Date);
                                boqDetailsCmd.Parameters.AddWithValue("@createdBy", userId);
                                boqDetailsCmd.Parameters.AddWithValue("@createdDate", DateTime.Now);
                                boqDetailsCmd.Parameters.AddWithValue("@refId", boqItemId); // ✅ parent BOQ id

                                int boqDetailId = Convert.ToInt32(await boqDetailsCmd.ExecuteScalarAsync());

                                if (boqDetailId <= 0)
                                    throw new Exception($"Failed to insert boq_details for assembly: {asmName}");

                                // ── Step 2: Insert tbl_items (Inventory Part) ────────
                                // ✅ FIX: Two separate queries to reliably get the ID
                                // First try to insert (skip if name exists)
                            ...
                            SELECT
                                @code, @warehouseId, '11 - Inventory Part', 0, @name, 0,
                                '', @costPrice, @cogsAccountId, @vendorId,
                                '0', @incomeAccountId, @assetAccountId,
                                0, 0, @onHand, 'fifo', 0,
                                @date, '', 0, 0, @createdBy, @createdDate, 'Inventory'
                            WHERE NOT EXISTS (SELECT 1 FROM tbl_items WHERE name = @name);";

                                await using var insertItemCmd = new MySqlCommand(
                                    insertItemQuery, conn, transaction);
                                insertItemCmd.Parameters.AddWithValue("@code", itemCode.ToString());
                                insertItemCmd.Parameters.AddWithValue("@warehouseId", model.WarehouseId);
                                insertItemCmd.Parameters.AddWithValue("@name", asmName);
                                insertItemCmd.Parameters.AddWithValue("@costPrice", asmCost);
                                insertItemCmd.Parameters.AddWithValue("@cogsAccountId", assemblyItem.COGSAccountId);
                                insertItemCmd.Parameters.AddWithValue("@vendorId", assemblyItem.VendorAccountId);
                                insertItemCmd.Parameters.AddWithValue("@incomeAccountId", assemblyItem.IncomeAccountId);
                                insertItemCmd.Parameters.AddWithValue("@assetAccountId", assemblyItem.AssetAccountId);
                                insertItemCmd.Parameters.AddWithValue("@onHand", asmQty);
                                insertItemCmd.Parameters.AddWithValue("@date", model.Date);
                                insertItemCmd.Parameters.AddWithValue("@createdBy", userId);
                                insertItemCmd.Parameters.AddWithValue("@createdDate", DateTime.Now.Date);
                                await insertItemCmd.ExecuteNonQueryAsync();

                                // ✅ FIX: Always fetch by name — reliable regardless of insert/skip
                                await using var getItemIdCmd = new MySqlCommand(
                                    "SELECT id FROM tbl_items WHERE name = @name LIMIT 1;",
                                    conn, transaction);
                                getItemIdCmd.Parameters.AddWithValue("@name", asmName);
                                int inventoryItemId = Convert.ToInt32(
                                    await getItemIdCmd.ExecuteScalarAsync() ?? 0);

                                if (inventoryItemId > 0) itemCode++;

                                Console.WriteLine($"  → inventoryItemId:{inventoryItemId} boqDetailId:{boqDetailId} boqItemId:{boqItemId}");

                                // ── Step 3: Insert tbl_item_assembly_bos ────────────
                                // assembly_id = parent BOQ row, item_id = boq_detail row
                                await using var bosCmd = new MySqlCommand(@"
                            INSERT INTO tbl_item_assembly_bos(assembly_id, item_id, qty)
                            VALUES (@assemblyId, @itemId, @qty);",
                                    conn, transaction);
                                bosCmd.Parameters.AddWithValue("@assemblyId", boqItemId);   // ✅ BOQ parent
                                bosCmd.Parameters.AddWithValue("@itemId", boqDetailId); // ✅ detail row
                                bosCmd.Parameters.AddWithValue("@qty", asmQty);
                                await bosCmd.ExecuteNonQueryAsync();

                                // ── Step 4: Insert tbl_item_assembly ────────────────
                                if (inventoryItemId > 0)
                                {
                                    await using var asmRelCmd = new MySqlCommand(@"
                                INSERT INTO tbl_item_assembly(assembly_id, item_id, qty)
                                VALUES (@assemblyId, @itemId, @qty);",
                                        conn, transaction);
                                    asmRelCmd.Parameters.AddWithValue("@assemblyId", inventoryItemId);
                                    asmRelCmd.Parameters.AddWithValue("@itemId", boqDetailId);
                                    asmRelCmd.Parameters.AddWithValue("@qty", asmQty);
                                    await asmRelCmd.ExecuteNonQueryAsync();
                                }
                        ...
                        {
                            Console.WriteLine($"[Assembly] Item '{description}' has NO assembly items");
                        }
                    }
                ...
                {
                    await transaction.RollbackAsync();
                    Console.WriteLine($"[SaveOrUpdateEstimation] INNER ERROR: {ex.Message}\n{ex.StackTrace}");
                    throw;
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine($"[SaveOrUpdateEstimation] OUTER ERROR: {ex.Message}");
                return StatusCode(500, new
                {
                    status = false,
                    message = "An unexpected error occurred: " + ex.Message
