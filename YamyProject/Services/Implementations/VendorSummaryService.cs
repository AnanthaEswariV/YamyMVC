namespace YamyProject.Services.Implementations
    {
    public class VendorSummaryService(YamyDbContext context) : IVendorSummaryService
        {
        public YamyDbContext _context = context;

        public async Task<List<CustomerSummaryRowViewModel>> GetVendorBalancesAsync(/*DateOnly? dateFrom, DateOnly? dateTo*/)
            {
      ...
      _context.TblTransactions
        ...
        public async Task<List<CustomerSummaryBalanceDetailRowViewModel>> GetVendorStatementAsync(int humId/*,DateOnly? dateFrom, DateOnly? dateTo*/)
            {
            var allowedTypes = new[]
             {
                   "Vendor Payment",
      ...
      .ToListAsync();
        ...
        public async Task<CustomerAgingSummaryViewModel> GetVendorAgingAsync(DateOnly? dateFrom = null, DateOnly? dateTo = null)
            {
            var allowedTypes = new[]
            {
         "Vendor Payment",
            ...
            .Where(t =>
                t.State == 0 &&
                t.HumId != null && t.HumId == t.Vendors.Id &&
            ...
            var txList = await txQuery
                 .Where(t => t.HumId == t.Vendors.Id && t.HumId != 0)
                .Select(t => new
                    {
                    CustomerId = t.Vendors.Id,   // int?
                    CustomerCode = t.Vendors.Code,
                    CustomerName = t.Vendors.Name,
        ...
        public async Task<CustomerSalesListViewModel> GetVendorSalesAsync(int customerId, DateOnly? dateFrom, DateOnly? dateTo)
            {
            var allowedTypes = new[]
            {
             "Vendor Payment",
        ...
        public async Task<CustomerBalanceSummaryViewModel> GetVendorBalancesSummryAsync()
            {
        ...
        public async Task<CustomerBalanceDetailViewModel> GetVendorDetailsStatementAsync(int humId, DateOnly? dateFrom = null, DateOnly? dateTo = null)
            {
            var allowedTypes = new[]
            {
        "Vendor Payment",
