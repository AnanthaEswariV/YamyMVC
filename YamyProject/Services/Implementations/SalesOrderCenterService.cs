namespace YamyProject.Services.Implementations
    ...
    public class SalesOrderCenterService(YamyDbContext context, IListServices listServices) : ISalesOrderCenterService
        ...
        public async Task<IEnumerable<SalesCenterViewModel>> GetDefaultReportAsync()
            ...
            var query = _context.TblSalesOrders
              .Where(s => s.State == 0)
        ...
        public async Task<IEnumerable<SalesCenterViewModel>> GetDetailedReportAsync()
            ...
            var sales = await query
                .Select(s => new
                    {
                    Sale = s,
                    Details = _context.TblSalesOrderDetails
                                 .Where(sd => sd.SalesId == s.Id)
                                 .Select(sd => new
                                     {
                                     SalesDetails = sd,
                                     Items = _context.TblItems.FirstOrDefault(i => i.Id == sd.ItemId)
                                     }).ToList(),
    ...
    .SelectMany(x => x.Details.Select(d => new
        ...
        x.Sale.Vat,
        x.Sale.Net,
        Item_Name = (d.Items != null ? (d.Items.Code + " - " + d.Items.Name) : ""),
        d.SalesDetails.Qty,
    ...
    .ToList();
            ...
            foreach (var s in sales)
                ...
                foreach (var sd in s.Details)
                    ...
                    result.Add(new SalesCenterViewModel
                        ...
                        Vat = s.Sale.Vat,
                        Net = s.Sale.Net,
                        ItemName = sd.Items != null ? sd.Items.Code + " - " + sd.Items.Name : "",
                ...
                try
                    ...
                    foreach (var r in vm.Items.Where(i => i != null && i.ItemId.GetValueOrDefault() > 0))
                        ...
                        if (r.ItemId != null && r.ItemId != 0)
                            ...
                            details.Add(new TblSalesOrderDetail
                                {
                                SalesId = saleId,
                                ItemId = r.ItemId,
                                Qty = qty,
        ...
        public async Task<TaxInvoiceViewModel> GetEditAsync(int id)
            {
        ...
        private static TaxInvoiceViewModel MapToViewModel(TblSalesOrder s)
            {
            // Coalesce details to an empty list to avoid NRE on OrderBy/Select
        ...
        public async Task UpdateTaxInvoiceAsync(TaxInvoiceViewModel Model, int currentUserId)
            ...
            await strategy.ExecuteAsync(async () =>
            {
                ...
                try
                    {
                    var Sales = _context.TblSalesOrders.Find(Model.Id);
                    if (Sales == null)
                        { }
                    ...
                    var SalesDetails = await _context.TblSalesOrderDetails
                        .Where(d => d.SalesId == Model.Id)
                        .ToListAsync();
                    _context.TblSalesOrderDetails.RemoveRange(SalesDetails);
                    await _context.SaveChangesAsync();
