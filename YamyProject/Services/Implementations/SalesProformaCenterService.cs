namespace YamyProject.Services.Implementations
    ...
    public class SalesProformaCenterService(YamyDbContext context, IListServices listServices) : ISalesProformaCenterService
        ...
        public async Task<IEnumerable<SalesCenterViewModel>> GetDefaultReportAsync()
            ...
            var query = _context.TblSalesProformas
              .Where(s => s.State == 0)
        ...
        public async Task<IEnumerable<SalesCenterViewModel>> GetDetailedReportAsync()
            ...
            var sales = await query
                .Select(s => new
                    {
                    Sale = s,
                    Details = _context.TblSalesProformaDetails
    ...
    .ToList();
      ...
      //method to create quotation from proforma + New quotation
        public async Task CreateProformaCenterAsync(TaxInvoiceViewModel vm, int currentUserId)
            ...
            await strategy.ExecuteAsync(async () =>
            {
                ...
                try
                    ...
                    _context.TblSalesProformas.Add(sale);
                    await _context.SaveChangesAsync(); // need sale.Id
                    var saleId = sale.Id;
                    var details = new List<TblSalesProformaDetail>();
                    ...
                    foreach (var r in vm.Items.Where(i => i != null && i.ItemId.GetValueOrDefault() > 0))
                        ...
                        if (r.ItemId != null && r.ItemId != 0)
                            ...
                            details.Add(new TblSalesProformaDetail
                                {
                                SalesId = saleId,
                                ItemId = r.ItemId,
                                Qty = qty,
        ...
        private static TaxInvoiceViewModel MapToViewModel(TblSalesProforma s)
            {
        ...
        public async Task UpdateProformaInvoiceAsync(TaxInvoiceViewModel Model, int currentUserId)
            ...
            await strategy.ExecuteAsync(async () =>
            {
                ...
                try
                    {
                    var Sales = _context.TblSalesProformas.Find(Model.Id);
                    if (Sales == null)
                        { }
                    ...
                    var SalesDetails = await _context.TblSalesProformaDetails
                        .Where(d => d.SalesId == Model.Id)
                        .ToListAsync();
                    _context.TblSalesProformaDetails.RemoveRange(SalesDetails);
                    await _context.SaveChangesAsync();
